...
describe('Excel', () => {
    ...
    it('should add a sheet and retrieve its data', async () => {
        ...
        expect(fs.existsSync(targetFilePath)).toBe(true); // Check if the file exists
    ...
    it('should update sheet data', () => {
        ...
        excelBase['setData'](sheetName, initialData, 'table1');
        excelBase['setData'](sheetName, updatedData, 'table2'); // Accessing protected method

        const retrievedData = excelBase['getData'](sheetName, 'table2');
        expect(retrievedData).toMatchObject(updatedData);
    });
    ...
    it('should read from an actual XLSX file', async () => {
        ...
        expect(sheetData).toBe(typeof Array);
